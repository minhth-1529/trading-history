...
const stringToHexColor = (
  ...
  if (colorDictionary?.[dictionary]?.[category]?.[value])
    color = colorDictionary?.[dictionary][category][value];
  if (category === "color")
    color = colorDictionary?.[dictionary]?.[category]?.[value] ?? "#fff";
  return color;
};
...
export const hexToRgbA = (hex, opacity) => {
  ...
  if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) {
    ...
    c = "0x" + c.join("");
    return (
      "rgba(" + [(c >> 16) & 255, (c >> 8) & 255, c & 255, op].join(",") + ")"
    );
  }
